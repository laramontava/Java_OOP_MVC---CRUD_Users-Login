...
public class functions_users {
	...
	public static admin pideadmin(configuration conf, idioma translate){
		...
		date_birthday=kernel.datebirthday(translate.getProperty("datebirthday")+formatdate,"Date Birthday",conf,translate);
		...
		hiring_date=kernel.yearsservice(translate.getProperty("hiring_date")+formatdate,"Hiring date",date_birthday,conf,translate);
	...
	public static client pideclient(configuration conf, idioma translate){
		String dni, name, subname, mobile, email, date_birthday, user, pass, avatar, state;
		float shopping;
		...
		avatar=functions.validatestring(translate.getProperty("avatar"), "Avatar");
		state=menus.menustate(translate.getProperty("state"), "State");
		up_date=kernel.yearsservice(translate.getProperty("up_date")+formatdate,"Hiring date",date_birthday,conf,translate);
	...
	public static registered_user pideregistrado(configuration conf, idioma translate){
		String dni, name, subname, mobile, email, date_birthday, user, pass, avatar, state;
		int activity;
		String formatdate="";
		switch(conf.getDate()){
	...
	public static void savedniadmin (admin admind, configuration conf,idioma translate, users user){
		int pos = -1;
		admind = functions_users.finddniadmin(translate);
		pos = Arraylist_admin.adminfind(admind, conf, translate);
		if (pos != -1) {
	...
	public static void savedniclient (client clientd, configuration conf,idioma translate, users user){
		int pos = -1;
		clientd = functions_users.finddniclient(translate);
		pos = Arraylist_client.ClientFind(clientd, conf, translate);
		if (pos != -1) {
			JOptionPane.showMessageDialog(null, "Error");
		}else {
			user.setDni(singleton.dni);
	...
	public static void savednireguser (registered_user ruserd, configuration conf,idioma translate, users user){
		int pos = -1;
		ruserd = functions_users.finddniuserreg(translate);
		pos = Arraylist_userreg.UserregFind(ruserd, conf, translate);
		if (pos != -1) {
			JOptionPane.showMessageDialog(null, "Error");
		}else {
			user.setDni(singleton.dni);
	...
	public static void cambiauser(users type, configuration conf, idioma translate){
		int option3;
		String formatdate="";
		...
		if(type instanceof admin){
			...
			case 1:
				...
				savedniadmin(admins, conf, translate, type);
				break;
			case 2:
			...
			case 5:
				...
				break;
			case 6:
				type.setDate_birthday(kernel.datebirthday(translate.getProperty("datebirthday")+formatdate,"",conf,translate));
				break;
			case 7:
			...
			case 10:
				...
				break;
			case 11:
				((admin)type).setHiring_date(kernel.yearsservice(translate.getProperty("hiring_date")+formatdate, "Up date",type.getDate_birthday(),conf,translate));
				break;
			case 12:
		...
		if(type instanceof client){
			...
			case 1:
				//type.setDni(kernel.pedirdatos("dni",translate.getProperty("dni"),"DNI", translate));
				savedniclient(clients, conf, translate, type);
				break;
			case 2:
				type.setName(kernel.pedirdatos("name",translate.getProperty("name"),"Name", translate));
				break;
			...
			case 10:
				...
				break;
			case 11:
				((client)type).setUp_date(kernel.yearsservice(translate.getProperty("up_date")+formatdate, "Up date",type.getDate_birthday(),conf,translate));
				break;
			case 12:
		...
		if(type instanceof registered_user){
			...
			case 1:
				//type.setDni(kernel.pedirdatos("dni",translate.getProperty("dni"),"DNI", translate));
				savednireguser(rusers, conf, translate, type);
				break;
			case 2:
				type.setName(kernel.pedirdatos("name",translate.getProperty("name"),"Name", translate));
				break;
