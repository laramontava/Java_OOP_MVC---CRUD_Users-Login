...
public class admin extends users implements Serializable{
	...
	public void currency(configuration conf, char moneda){
		float cambia;
		
		if(moneda=='€'&&conf.getCurrency()=='$'){//eur a usd *1,0934
			cambia=(float) (this.salary*1.0934);
			this.salary=cambia;
		}
		if(moneda=='$'&&conf.getCurrency()=='€'){//usd a eur /1,0934
			cambia=(float) (this.salary/1.0934);
			this.salary=cambia;
		}
		if(moneda=='€'&&conf.getCurrency()=='£'){//eur a libra *0,728
			cambia=(float) (this.salary*0.728);
			this.salary=cambia;
		}
		if(moneda=='£'&&conf.getCurrency()=='€'){//libra a eur /0,728
			cambia=(float) (this.salary/0.728);
			this.salary=cambia;
		}
		if(moneda=='$'&&conf.getCurrency()=='£'){//usd a libra *0,6658
			cambia=(float) (this.salary*0.6658);
			this.salary=cambia;
		}
		if(moneda=='£'&&conf.getCurrency()=='$'){//libra a usd /0,6658
			cambia=(float) (this.salary/0.6658);
			this.salary=cambia;
		}
	}
